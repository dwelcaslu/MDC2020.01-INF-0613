...
# price
carros$price<-as.numeric(levels(carros$price))[carros$price]
carros$price[is.na(carros$price)]=mean(carros$price,na.rm=TRUE)

# bore -- calibre
carros$bore<-as.numeric(levels(carros$bore))[carros$bore]
carros$bore[is.na(carros$bore)]=mean(carros$bore,na.rm=TRUE)

# stroke
carros$stroke<-as.numeric(levels(carros$stroke))[carros$stroke]
carros$stroke[is.na(carros$stroke)]=mean(carros$stroke,na.rm=TRUE)

# horsepower
carros$horsepower<-as.numeric(levels(carros$horsepower))[carros$horsepower]
carros$horsepower[is.na(carros$horsepower)]=mean(carros$horsepower,na.rm=TRUE)

# peak_rpm
carros$peak_rpm<-as.numeric(levels(carros$peak_rpm))[carros$peak_rpm]
